...
export class DialogoLoginComponent {
  ...
  readonly formulario = this.construtorFormulario.nonNullable.group({
    email: ['cliente@exemplo.com', [Validators.required, Validators.email]],
  ...
  entrar(): void {
    ...
    this.autenticacao.autenticar(email, senha).pipe(finalize(() => this.carregando.set(false))).subscribe({
      next: () => this.referencia.close(),
      error: () => this.erro.set('Não foi possível entrar. Confira suas credenciais e tente novamente.')
    });
  }
}
