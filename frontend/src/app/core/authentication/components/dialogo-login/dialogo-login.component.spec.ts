...
describe('DialogoLoginComponent', () => {
  ...
  it('autentica com as credenciais preenchidas e fecha o diálogo', () => {
    autenticacao.autenticar.mockReturnValue(of({ token: 'token', usuario: { id: '1', nome: 'Cliente', email: 'cliente@exemplo.com' } }));

    componente.entrar();

    expect(autenticacao.autenticar).toHaveBeenCalledWith('cliente@exemplo.com', '123456');
  ...
  it('exibe uma mensagem quando a autenticação falha', () => {
    autenticacao.autenticar.mockReturnValue(throwError(() => new Error('Credenciais inválidas')));

    componente.entrar();

    expect(componente.erro()).toBe('Não foi possível entrar. Confira suas credenciais e tente novamente.');
    expect(componente.carregando()).toBe(false);
