...
import { DadosSimulacao, PropostaSimulada, TipoPagamento } from '../../core/models/proposal.model';
...
@Component({
  ...
  imports: [CurrencyPipe, DatePipe, ReactiveFormsModule, RouterLink, MatButtonModule, MatCardModule, MatDatepickerModule, MatFormFieldModule, MatInputModule, MatNativeDateModule, MatRadioModule, MatSelectModule, MatProgressSpinnerModule],
...
export class SimuladorPropostaComponent {
  ...
  readonly dividaSelecionada = computed<Divida | null>(() => this.dividas.dividas().find((divida) => divida.id === this.idDivida()) ?? null);
  readonly formulario = this.construtorFormulario.nonNullable.group({
    ...
    dataPrimeiroVencimento: [this.adicionarDias(this.hoje, 30), Validators.required]
  });

  constructor() {
    this.rota.queryParamMap.subscribe((parametros) => this.idDivida.set(parametros.get('divida')));
  }
  ...
  simular(): void {
    ...
    const dados: DadosSimulacao = {
      ...
      quantidadeParcelas: valores.tipoPagamento === 'PARCELADO' ? valores.quantidadeParcelas : undefined,
      dataPrimeiroVencimento: valores.dataPrimeiroVencimento.toISOString().slice(0, 10)
    };
    this.propostas.simular(dados).pipe(finalize(() => this.carregando.set(false))).subscribe({
      next: (proposta) => this.proposta.set(proposta),
      error: () => this.erro.set('Não foi possível calcular a proposta. Tente novamente.')
    });
  }
