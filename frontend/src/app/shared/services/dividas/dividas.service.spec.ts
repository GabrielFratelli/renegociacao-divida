...
describe('DividasService', () => {
  ...
  it('deve carregar e disponibilizar as dívidas com signal', () => {
    ...
    requisicao.flush([{ id: 'd-1', credor: 'Banco Aurora', descricao: 'Cartão', valorOriginal: 500, vencimento: '2026-05-10', status: 'ATRASADA' }]);

    expect(servico.carregando()).toBe(false);
    expect(servico.dividas()).toHaveLength(1);
    expect(servico.dividas()[0].credor).toBe('Banco Aurora');
  });
});
