...
export class DividasService {
  ...
  carregar(): void {
    ...
    this.erro.set(null);
    this.http.get<Divida[]>(`${ambiente.apiUrl}/dividas`).pipe(finalize(() => this.carregando.set(false))).subscribe({
      next: (dividas) => this.dividas.set(dividas),
      error: () => this.erro.set('Não foi possível carregar suas dívidas. Tente novamente.')
    });
  }
}
