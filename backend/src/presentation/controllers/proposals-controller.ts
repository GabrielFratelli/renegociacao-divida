...
import { z } from 'zod';
import { DividaNaoEncontradaError, SimularProposta } from '../../application/use-cases/simulate-proposal.js';
...
const esquemaSimulacao = z.object({
  ...
  if (dados.tipoPagamento === 'PARCELADO' && !dados.quantidadeParcelas) {
    contexto.addIssue({ code: 'custom', message: 'Informe a quantidade de parcelas.', path: ['quantidadeParcelas'] });
  }
});
...
export class ControladorPropostas {
  ...
  simular = async (requisicao: RequisicaoAutenticada, resposta: Response): Promise<void> => {
    ...
    if (!dados.success) {
      resposta.status(400).json({ mensagem: 'Parâmetros de simulação inválidos.', detalhes: dados.error.flatten() });
      return;
    }
    try {
      const proposta = await this.simularProposta.executar(requisicao.usuario!.id, dados.data);
