...
export const criarAplicacao = (configuracao: ConfiguracaoAplicacao): Express => {
  ...
  const controladorDividas = new ControladorDividas(new ListarDividas(repositorioDividas));
  const controladorPropostas = new ControladorPropostas(new SimularProposta(repositorioDividas));

  aplicacao.disable('x-powered-by');
  aplicacao.use(helmet());
  aplicacao.use(cors({ origin: configuracao.origemPermitida, methods: ['GET', 'POST'], allowedHeaders: ['Content-Type', 'Authorization'] }));
  aplicacao.use(express.json({ limit: '30kb' }));
  aplicacao.use('/api', rateLimit({ windowMs: 60_000, limit: 100, standardHeaders: 'draft-8', legacyHeaders: false }));

  aplicacao.get('/api/saude', (_: Request, resposta: Response) => resposta.json({ status: 'ok' }));
  aplicacao.post('/api/auth/login', controladorAutenticacao.login);
  aplicacao.get('/api/dividas', autenticar(servicoToken), controladorDividas.listar);
  aplicacao.post('/api/propostas/simular', autenticar(servicoToken), controladorPropostas.simular);
  aplicacao.use((erro: unknown, _: Request, resposta: Response, __: NextFunction) => {
    console.error(erro);
