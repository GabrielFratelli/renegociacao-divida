...
describe('SimularProposta', () => {
  ...
  it('deve aplicar 18% de desconto na proposta à vista', async () => {
    const proposta = await casoDeUso.executar('cliente-001', { dividaId: 'div-001', tipoPagamento: 'A_VISTA', dataPrimeiroVencimento: '2026-09-01' });
  ...
  it('deve dividir o valor com desconto para até seis parcelas', async () => {
    const proposta = await casoDeUso.executar('cliente-001', { dividaId: 'div-002', tipoPagamento: 'PARCELADO', quantidadeParcelas: 4, dataPrimeiroVencimento: '2026-09-01' });
